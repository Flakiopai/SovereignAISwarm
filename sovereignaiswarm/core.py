...
class Swarm:
    def __init__(self, client=None, config: Optional[SovereignConfig] = None):
        self.config = config or load_config()
        if client is None:
            client = LocalLLM(config=self.config)
        self.client = client
            ...
            case _:
                try:
                    ...
                    error_message = (
                        ...
                        "Make sure agent functions return a string or Result object. "
                        f"Error: {str(e)}"
                    )
    ...
    def _effective_max_turns(self, max_turns: Union[int, float]) -> Union[int, float]:
        ceiling = self.config.max_turns
        if max_turns == float("inf"):
            return ceiling
        return min(max_turns, ceiling)
    ...
    def run_and_stream(
        ...
        max_turns = self._effective_max_turns(max_turns)
